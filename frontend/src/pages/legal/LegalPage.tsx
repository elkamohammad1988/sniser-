    ...
    <>
      <Section tone="dark" spacing="md">
        ...
        <p className="mt-3 text-xs text-white/40">
          {t("legal.shell.effective", { date: effective })}
        </p>
        {summary && (
      ...
      <Section tone="dark" spacing="sm">
        ...
        <div className="mx-auto mt-12 max-w-3xl rounded-2xl bg-bg-card p-6 ring-1 ring-white/5 sm:flex sm:items-center sm:justify-between sm:gap-6">
          <div>
            <h3 className="text-base font-bold text-white">{t("legal.shell.needDifferentTitle")}</h3>
            <p className="mt-1 text-sm text-white/65">
              {t("legal.shell.needDifferentBody")}
            </p>
          </div>
          <div className="mt-4 sm:mt-0 sm:shrink-0">
            <Link to="/contact">
              <Button variant="primary" size="sm">{t("legal.shell.contactLegal")}</Button>
            </Link>
          </div>
        </div>
      </Section>
